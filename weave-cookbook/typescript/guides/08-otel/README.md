...
On Dedicated Cloud or self-managed instances the base URL is `https://<your-instance>.wandb.io/traces` instead. Note the exporter package: W&B's endpoint only accepts protobuf, so the example uses `@opentelemetry/exporter-trace-otlp-proto`, not the HTTP/JSON exporter.

## How it works

[`examples/08-otel.ts`](../../examples/08-otel.ts) builds an `OTLPTraceExporter` pointed at the endpoint and attaches it to a `NodeTracerProvider` through a `BatchSpanProcessor` (the recommended processor — `SimpleSpanProcessor` exports synchronously and drags on your workload). `weave` is never imported:
...
The catalog search then runs inside two ordinary OTel spans, `recommend_game` and `search_catalog`. `await provider.shutdown()` at the end matters: the batch processor flushes asynchronously, and a process that exits first drops spans without an error.

> **The endpoint pitfall.** The example passes the full path to `url:` on the exporter, which uses it verbatim. If you configure the standard `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable instead, the SDK appends `/v1/traces` on its own — so that variable must hold the base `https://trace.wandb.ai/otel`, or the request goes to a doubled path, gets a 404, and the spans are silently dropped.
...
Full capture: [typescript-08-otel.txt](../../../assets/expected-output/typescript-08-otel.txt).
